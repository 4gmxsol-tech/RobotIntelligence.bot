...
function renderKnowledgeGraph(){
 ...
 const pos={};const layerMap={COMPANY:0,MODEL:1,ROBOT:2,CAPABILITY:3,SIGNAL:4};[0,1,2,3,4].forEach(layer=>{const group=nodes.filter(n=>layerMap[n.type]===layer);const x=80+layer*((W-160)/4),gap=Math.max(68,(H-90)/Math.max(group.length,1));group.forEach((n,i)=>pos[n.id]={x,y:55+i*gap});});
 const svg=(visibleIds)=>'<svg viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="xMidYMid meet"><g transform="translate('+offsetX+' '+offsetY+') scale('+scale+')">'+edges.filter(e=>!visibleIds||(visibleIds.has(e.a)&&visibleIds.has(e.b))).map(e=>'<line class="g-edge '+(selected&&(e.a===selected||e.b===selected)?"hot":"")+'" x1="'+pos[e.a].x+'" y1="'+pos[e.a].y+'" x2="'+pos[e.b].x+'" y2="'+pos[e.b].y+'"/>').join("")+nodes.filter(n=>!visibleIds||visibleIds.has(n.id)).map(n=>'<g class="g-node '+(selected===n.id?"selected":"")+'" data-id="'+esc(n.id)+'" transform="translate('+pos[n.id].x+' '+pos[n.id].y+')"><rect class="g-node-box" x="-9" y="-18" width="18" height="36" rx="5" fill="#091820" stroke="'+palette[n.type]+'" stroke-width="1"/><circle r="4" fill="'+palette[n.type]+'"/><text class="g-label" x="15" y="-2">'+esc(n.label.length>20?n.label.slice(0,19)+"…":n.label)+'</text><text class="g-type" x="15" y="10">'+esc(n.type)+'</text></g>').join("")+'</g></svg>';
 let visibleIds=null;
